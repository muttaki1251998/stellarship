...
const UploadSoldListing = ({ onSoldListingUploaded }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.NEXT_BACKEND_API_URL}/upload-soldListing`,
        formDataToSend,
        {
          headers: {
      ...
      onSoldListingUploaded(); // Fetch the latest listings after a new one is uploaded
    } catch (error) {
  ...
  return (
    <div className="bg-white p-8 rounded-lg shadow-lg mb-8">
      <h2 className="text-2xl font-bold mb-6">Upload Sold Listing</h2>
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">Listing Images (up to 15)</label>
          <input
            type="file"
        ...
        <div className="grid grid-cols-3 gap-4 mt-4">
          {listingData.soldListingImages.length > 0 &&
            Array.from(listingData.soldListingImages).map((file, index) => (
              <img
                key={index}
      ...
      >
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1">Listing Uploaded</ModalHeader>
              <ModalBody>
...
export default UploadSoldListing;
